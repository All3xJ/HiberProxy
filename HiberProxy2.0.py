...
def proxyget(url): # scarica proxy da altri siti
	try:
		...
		print("\nAn error occurred, skipping to the next website.") # printa questo
...
def proxychecker(i):
	...
	try:
		urllib.request.urlopen(req, timeout=20)						# apre il sito
...
def main(): # funzione effettiva del programma.
	try:
		...
		for position, url in enumerate(nurls):
			threading.Thread(target=proxyget, args=[url]).start() # avvia un thread per url per velocizzare enormemente il processo.
